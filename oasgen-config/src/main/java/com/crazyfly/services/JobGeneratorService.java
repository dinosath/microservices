...
public class JobGeneratorService {
    ...
    @Inject
    private OasGenTemplateCrudService oasGenTemplateCrudService;

    public void generateJob(String oasUrl, int oasGenTemplateId) {

        oasGenTemplateCrudService.getOasGenTemplate(oasGenTemplateId)
                .onItem().invoke(oasGenTemplate -> {
                    final Job job = createJobResource(oasUrl, oasGenTemplate);
    ...
    private Job createJobResource(String oasUrl, OasGenTemplate oasGenTemplate) {
        return new JobBuilder()
                ...
                .withArgs("generate", "-i", oasUrl, "-c", "/data/quarkus/config.yaml", "-o", "/data/out")
                .addNewVolumeMount()
                .withName("oasgen")
                .withMountPath("/data")
                .endVolumeMount()
